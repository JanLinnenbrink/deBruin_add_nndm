...
source("/home/j/jlinnenb/deBruin_add_nndm/R/bnndm_dev.R")
#source("C:/0_Msc_Loek/M7_Fernerkundung/NNDMpaper-main/code/bnndm_dev.R")
...
if(!dir.exists(paste0(outfolder, "/bnndm_def")))
  dir.create(paste0(outfolder, "/bnndm_def"))
...
bnndmCV <- function(smpl, number, variate, seed){
  ...
  for(i_CV in 1:n_CV){
    ...
    set.seed(seed)
    time[i_CV] <- system.time(bnndm_res <- bnndm(tpoints=pts_sf, ppoints=ppoints))[[3]]
    

    WS[i_CV] <- bnndm_res$stat
    fold <- bnndm_res$blocks
    
    set.seed(seed)
    time_mod[i_CV] <- system.time(for(k in 1:length(unique(fold[fold>0]))){
      
       if(variate == "AGB"){
        RFmodel <- ranger(agb~., AGBdata[fold != k & fold > 0,], 
                          respect.unordered.factors=TRUE)
        refs <- AGBdata$agb[fold == k & fold > 0] 
        preds  <- predict(RFmodel, AGBdata[fold == k & fold > 0,])$predictions
      } else{
  ...
  f_out  <- file.path(outfolder,"bnndm_def", fname)
...
mclapply(seq(n_samp), function(i) {
  for(smpl in samples) {
    bnndmCV(smpl, i, "AGB", startseed)
    bnndmCV(smpl, i, "OCS", startseed)
  }
}, mc.cores = cores)

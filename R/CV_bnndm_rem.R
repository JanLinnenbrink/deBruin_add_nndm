...
if(!dir.exists(paste0(outfolder, "/bnndm_100_rem")))
  dir.create(paste0(outfolder, "/bnndm_100_rem"))
...
bnndmCV <- function(smpl, number, variate, seed){
  ...
  for(i_CV in 1:n_CV){
    ...
    set.seed(seed)
    time[i_CV] <- system.time(bnndm <- bnndm(pts_sf, ppoints = ppoints, rm=TRUE, n_steps=100))[[3]]
    WS[i_CV] <- bnndm$stat
    fold <- bnndm$blocks
    f_rem <- bnndm$indx_rem
    
    set.seed(seed)
    for(k in 1:length(unique(fold))){
      if(variate == "AGB"){
        RFmodel <- ranger(agb~., AGBdata[fold != k i & fold != f_rem,], 
                          respect.unordered.factors=TRUE)
        refs <- AGBdata$agb[fold == k & fold != f_rem] 
        preds  <- predict(RFmodel, AGBdata[fold == k & fold != f_rem,])$predictions
      } else{
        RFmodel <- ranger(ocs~., OCSdata[fold != k & fold != f_rem,], 
                          respect.unordered.factors=TRUE)
        refs <- OCSdata$ocs[fold == k] 
        preds  <- predict(RFmodel, OCSdata[fold == k & fold != f_rem,])$predictions
      }
  ...
  f_out  <- file.path(outfolder,"bnndm_100_rem", fname)

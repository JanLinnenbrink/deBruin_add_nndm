...
if(!dir.exists(paste0(outfolder, "/knndm_sample_global_tuned")))
  dir.create(paste0(outfolder, "/knndm_sample_global_tuned"))
...
knndmCV <- function(smpl, number, variate, seed){
  ...
  f_out  <- file.path(outfolder,"knndm_sample_global_tuned", fname)
    ...
    if(variate == "AGB"){
      ...
      RFmodel <- caret::train(agb~., AGBdata,respect.unordered.factors=TRUE, method = "ranger",
                              trControl = trainControl(method = "cv",
                                                       index = kndm$indx_train, indexOut = kndm$indx_test,

...
randCV <- function(smpl, number, variate, seed){
  ...
  for(i_CV in 1:n_CV){
    ...
    if(variate == "AGB"){
      ...
      RFmodel <- caret::train(agb~., AGBdata,respect.unordered.factors=TRUE, method = "ranger",
                                   tuneGrid=pgrid, num.trees=500, importance="permutation",
                                   trControl = trainControl(method = "cv", savePredictions = "final"))
      AGBstack <- rast(file.path(infolder1, "AGBstack.tif"))
      cls <- data.frame(id=1:8, cover=c("1", "2", "3", "4", "5", "6","7","8"))
      levels(AGBstack$glc2017) <- cls
      ...
      tdi <- trainDI(RFmodel)
      AOA_AGB <- aoa(newdata = AGBstack, trainDI = tdi)
      
      vimp <- varImp(RFmodel) |>  c() 
      vimp <-  vimp$importance
      vimp <- data.frame(variable=rownames(vimp), weight = vimp$Overall)
      trainDI(train = RFmodel$trainingData[,2:ncol(RFmodel$trainingData)],
              weight = vimp,
              CVtest = RFmodel$control$indexOut)
      
    } else {
      if (is.na(vimp)[1]) print("no")
      nrow(vimp) != 1 || ncol(vimp) != length(vimp)
    ...
    err_rand <- global_validation(RFmodel)
    RMSE[i_CV] <- err_rand[[1]]
...
mclapply(seq(n_samp), function(i) {
  for(smpl in samples) {
    randCV(smpl, i, "AGB", startseed)
    randCV(smpl, i, "OCS", startseed)
  }
}, mc.cores = cores)

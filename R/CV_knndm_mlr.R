...
if(!dir.exists(paste0(outfolder, "/knndm_mlr")))
  dir.create(paste0(outfolder, "/knndm_mlr"))
...
knndmCV <- function(smpl, number, variate, seed){
  ...
  f_out  <- file.path(outfolder,"knndm_mlr", fname)
  ...
  for(i_CV in 1:n_CV){
    ...
    folds = as.factor(fold)
    custom_cv$instantiate(task_regr, f = folds)

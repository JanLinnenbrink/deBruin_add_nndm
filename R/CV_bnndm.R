...
if(!dir.exists(paste0(outfolder, "/bnndm_5")))
  dir.create(paste0(outfolder, "/bnndm_5"))
...
bnndmCV <- function(smpl, number, variate, seed){
  ...
  for(i_CV in 1:n_CV){
    ...
    set.seed(seed)
    time[i_CV] <- system.time(bnndm <- bnndm(pts_sf, ppoints = ppoints, rm=FALSE, n_steps=5, k=2:10))[[3]]
    WS[i_CV] <- bnndm$stat
    fold <- bnndm$blocks
  ...
  f_out  <- file.path(outfolder,"bnndm_5", fname)

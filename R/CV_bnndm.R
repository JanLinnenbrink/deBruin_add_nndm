...
outfolder <- "/scratch/tmp/jlinnenb/deBruin_add_nndm/CVresults/bnndm"
...
nndmCV <- function(smpl, number, variate) {
  ...
  for(i_CV in 1:n_CV) {
    ...
    bnndm <- bnndm(x = sample_subset, modeldomain = raster_sf, n_steps = 10, 
                   pmax_nan = 0.5, min_tpoints = 0.5, rm = TRUE)
    
    # Evaluate RF model using bNNDM CV
    trainControl_BNNDM <- trainControl(method = "cv",
                                      index=bnndm$indx_train,
                                      indexOut=bnndm$indx_test,
    ...
    if (!is.null(bnndm$indx_rem)) {
      training_data <- training_data[-bnndm$indx_rem,]
    } 
    else {
    ...
    mod_BNNDM <- train(rf_form,
                      method = "ranger",
                      trControl = trainControl_BNNDM,
                      tuneGrid = paramGrid, 
                      data = training_data)
    
    RMSE[i_CV] <- global_validation(mod_BNNDM)["RMSE"][[1]]
    folds <- append(folds, bnndm)
